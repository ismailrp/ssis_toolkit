...
import { cumulativeLeaders, durationLeaders } from "@/data/assessment";
...
export function AverageDurationChart({ accent }: { accent: string }) {
  const options: ApexOptions = { ...common, colors: [accent], plotOptions: { bar: { borderRadius: 4, horizontal: true, barHeight: "58%" } }, xaxis: { categories: durationLeaders.map(x => x.package), title: { text: "Average duration (detik)" } }, legend: { show: false }, tooltip: { y: { formatter: value => `${value.toLocaleString("id-ID", { maximumFractionDigits: 2 })} detik` } } };
...
export function CumulativeRuntimeChart({ accent }: { accent: string }) {
  const options: ApexOptions = { ...common, colors: [accent], plotOptions: { bar: { borderRadius: 4, horizontal: true, barHeight: "58%" } }, xaxis: { categories: cumulativeLeaders.map(x => x.package), title: { text: "Derived cumulative hours" } }, tooltip: { y: { formatter: value => `${value.toFixed(2)} jam` } }, legend: { show: false } };
