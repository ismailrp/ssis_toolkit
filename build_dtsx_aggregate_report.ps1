...
function Get-Categories([object]$row) { $items=New-Object System.Collections.Generic.List[string]; foreach($column in $categoryMap.Keys){$value=Get-Value $row $column; $number=0; if([int]::TryParse($value,[ref]$number) -and $number -gt 0){[void]$items.Add($categoryMap[$column])}}; $raw=Get-Value $row 'anti_pattern_fields'; foreach($field in ($raw -split ';')){if([string]::IsNullOrWhiteSpace($field)){continue};$category=([regex]::Replace($field,'(?<=[a-z0-9])(?=[A-Z])','-')).ToLowerInvariant();if(!$items.Contains($category)){[void]$items.Add($category)}};return @($items|Select-Object -Unique) }
